...
*CON_SIGNATURE_PRYN
keep SUBJID PREGNANCY_ID CON_YN_DSDECOD ESTIMATED_EDD_SCDAT SCRNID SCRN_OBSSTDAT AGE_IEORRES PC_IEORRES EXCL_YN_IEORRES ///
...
replace GEST_AGE_WKS_SCORRES= round(GEST_AGE_WKS_SCORRES)
	...
	*Labeling Variables
		label variable PREGNANCY_ID                 "Pregnancy ID"
	...
	*rename M01_CASEID CASEID
	...
	rename M01_PREGNANCY_ID PREGNANCY_ID
	********************
	*checking duplicates
     *bysort CASEID:  gen dup = cond(_N==1,0,_n)
	 bysort PREGNANCY_ID:  gen dup = cond(_N==1,0,_n)
	 ...
	 *keep SUBJID PREGNANCY_ID M01_SCRN_OBSSTDAT dup
	 *export excel using "ReMAPP_Duplicates_14_dec_2021_PID.xlsx" , sheet("MNH01") firstrow(variables) sheetmodify
...
keep  /// 
	...
	****/// No duplicates found in MNH_02
	 ...
	 *keep CASEID dup
	 *keep SUBJID dup
	 *export excel using "ReMAPP_Duplicates_14_dec_2021_PID.xlsx"  , sheet("MNH02") firstrow(variables) sheetmodify
	 *restore // restoring old dataset
	**********************
...
keep SUBJID PREGNANCY_ID PH_VISDAT PH_PREV_RPORRES PH_PREVN_RPORRES ///
	...
	*rename M03a_CASEID CASEID
	...
	rename M03a_PREGNANCY_ID PREGNANCY_ID
	********************
	*checking duplicates
     *bysort CASEID:  gen dup = cond(_N==1,0,_n)
	 bysort PREGNANCY_ID:  gen dup = cond(_N==1,0,_n)
	 ...
	 *keep if dup>0
	 *keep CASEID dup
	 *keep PREGNANCY_ID dup
	 *export excel using "ReMAPP_Duplicates_14_dec_2021_PID.xlsx"  , sheet("MNH03a") firstrow(variables) sheetmodify
	 *restore // restoring old dataset
	**********************
...
keep SUBJID PREGNANCY_ID PH_BS_RPORRES PH_METH LB_LENGTH_RPORRES BW_RPYN LOSS_ABORT_RPORRES LB_WEIGHT_RPORRES ///
	...
	*rename M03b_CASEID CASEID
	...
	rename M03b_PREGNANCY_ID PREGNANCY_ID
	********************
	*checking duplicates
     *bysort CASEID:  gen dup = cond(_N==1,0,_n)
	 bysort PREGNANCY_ID:  gen dup = cond(_N==1,0,_n)
	 ...
	 keep if dup>0
	 *keep CASEID dup
	 keep SUBJID PREGNANCY_ID dup
	 export excel using "ReMAPP_Duplicates_14_dec_2021_PID.xlsx"  , sheet("MNH03b") firstrow(variables) sheetmodify
	 restore // restoring old dataset
	**********************
...
keep SUBJID PREGNANCY_ID OBSSTDAT ANC_VISIT_N ANC_N_1 PATIENT_DSDECOD DTHDAT PRG_DSDECOD ///
	...
	*bysort CASEID (visit_date): gen anc_visit_num=_n 
	bysort PREGNANCY_ID (visit_date): gen anc_visit_num=_n
	*bysort CASEID (visit_date): gen anc_visit_tot=_N //total number of anc visits
	bysort PREGNANCY_ID (visit_date): gen anc_visit_tot=_N //total number of anc visits
	...
	*checking duplicates - this will be subjectid and visit date as we expect duplicates of subjectid
	 ...
	 *bysort CASEID visit_date: gen dup=cond(_N==1,0,_n) 
	 bysort PREGNANCY_ID visit_date: gen dup=cond(_N==1,0,_n)
	  duplicates report PREGNANCY_ID
	  ta dup
	 ...
	 *keep CASEID visit_date dup
	 keep SUBJID PREGNANCY_ID visit_date dup
	 export excel using "ReMAPP_Duplicates_14_dec_2021_PID.xlsx" , sheet("MNH04") firstrow(variables) sheetmodify
	 restore // restoring old dataset
	********************** 
	...
	*reshaping dataset from long to wide
	reshape wide visit_date SUBJID OBSSTDAT ANC_VISIT_N ANC_N_1 PATIENT_DSDECOD DTHDAT PRG_DSDECOD ///
	PRG_DSSTDAT PRG_DSSTERM PIH_MHOCCUR PIH_MHSTDAT PIH_MHTERM  HIV_MHOCCUR ///
	SYPH_MHOCCUR MALARIA_EVER_MHOCCUR DX_OTHR_PG_MHOCCUR DX_OTHR_LAST_VISIT_MHOCCUR ///
	FOLIC_ACID_CMOCCUR IRON_CMOCCUR CALCIUM_CMOCCUR ///
	MICRONUTRIENT_CMOCCUR ANTHELMINTHIC_CMOCCUR ///
	APH_RPORRES CARDIAC_EVER_MHOCCUR GEST_DIAB_RPORRES HTN_EVER_MHOCCUR LOWBIRTHWT_RPORRES ///
	MACROSOMIA_RPORRES OLIGOHYDRAMNIOS_RPORRES PREMATURE_RUPTURE_RPORRES PRETERM_RPORRES ///
	UNPL_CESARIAN_PROCCUR DIABETES_EVER_MHOCCUR PPH_RPORRES PREECLAMPSIA_RPORRES, i(PREGNANCY_ID)  j(anc_visit_num)  // not sure if this is the right way to call all variables
	...
	*rename M04_SUBJID SUBJID
	rename M04_PREGNANCY_ID PREGNANCY_ID
...
rename *, upper
*CASEID
keep SUBJID PREGNANCY_ID ANT_PEDAT ANT_VISIT_N WEIGHT_PEPERF WEIGHT_PERES HEIGHT_PEPERF HEIGHT_PERES MUAC_PEPERF MUAC_PERES 
*ANT_VISIT_N
 ...
 *Labeling Variables
	...
	*bysort CASEID (visit_date): gen ant_visit_num=_n 
	bysort PREGNANCY_ID (visit_date): gen ant_visit_num=_n 
	*bysort CASEID (visit_date): gen ant_visit_tot=_N //total number of anc visits
	bysort PREGNANCY_ID (visit_date): gen ant_visit_tot=_N //total number of anc visits
	********************
	*checking duplicates - this will be subjectid and visit date as we expect duplicates of subjectid
	 *bysort CASEID visit_date: gen dup=cond(_N==1,0,_n) 
	 bysort PREGNANCY_ID visit_date: gen dup=cond(_N==1,0,_n)
	 ...
	 *export excel using "ReMAPP_Duplicates_14_dec_2021_PID.xlsx"  , sheet("MNH05") firstrow(variables) sheetmodify
	 *restore // restoring old dataset
	********************** 
	...
	***** ANT_VISIT_N is also not constant with SUBJID 
	reshape wide ANT_VISIT_N visit_date ANT_PEDAT WEIGHT_PEPERF  WEIGHT_PERES HEIGHT_PEPERF HEIGHT_PERES MUAC_PEPERF MUAC_PERES , i(PREGNANCY_ID) j(ant_visit_num)
	...
	*rename M05_SUBJID SUBJID
	rename M05_PREGNANCY_ID PREGNANCY_ID
...
rename *, upper
*CASEID
keep SUBJID PREGNANCY_ID US_OHOSTDAT ANC_N_VISIT CRL1_FAORRES ///
 ...
 *Labeling Variables
	...
	*bysort CASEID (visit_date): gen us_visit_num=_n 
	bysort PREGNANCY_ID (visit_date): gen us_visit_num=_n
	*bysort CASEID (visit_date): gen us_visit_tot=_N //total number of anc visits
	bysort PREGNANCY_ID (visit_date): gen us_visit_tot=_N //total number of anc visits
	...
	*checking duplicates - this will be subjectid and visit date as we expect duplicates of subjectid
	 ...
	 *keep CASEID visit_date dup
	 keep SUBJID visit_date dup
	 export excel using "ReMAPP_Duplicates_14_dec_2021_PID.xlsx"  , sheet("MNH06") firstrow(variables) sheetmodify
	 restore // restoring old dataset
	********************** 
	...
	*reshaping dataset from long to wide
	...
	HC_FAORRES AC_FAORRES FL_FAORRES US_GA_WEEKS_AGE US_GA_DAYS_AGE ///
	US_EDD_BRTHDAT SINGLE_FAORRES NOT_SINGLE_FAORRES ///
	ABRUPTION_FAORRES PREVIA_FAORRES, i(PREGNANCY_ID)  j(us_visit_num) 
	...
	*rename M06_SUBJID SUBJID
	rename M06_PREGNANCY_ID PREGNANCY_ID
...
rename *, upper
*CASEID
keep SUBJID PREGNANCY_ID VISIT_OBSSTDAT ANC_N_VISIT CBC_SPCPERF CBC_HB_LBORRES	///
 ...
 *Labeling Variables
	...
	*bysort CASEID (visit_date): gen lab_visit_num=_n 
	bysort PREGNANCY_ID (visit_date): gen lab_visit_num=_n
	*bysort CASEID (visit_date): gen lab_visit_tot=_N //total number of anc visits
    bysort PREGNANCY_ID (visit_date): gen lab_visit_tot=_N //total number of anc visits
	...
	*checking duplicates - this will be subjectid and visit date as we expect duplicates of subjectid
	 ...
	 *eep if dup>0
	 *keep SUBJID visit_date dup
	 *export excel using "ReMAPP_Duplicates_14_dec_2021_PID"  , sheet("MNH07") firstrow(variables) sheetmodify
	 *restore // restoring old dataset
	********************** 
	...
	*reshaping dataset from long to wide
	...
	HIV_LBORRES	MALARIA_LBORRES	HEPB_SPCPERF HEPB_LBPERF HEPB_LBORRES HEPC_SPCPERF	///
	HEPC_LBPERF	HEPC_LBORRES UA_SPCPERF UA_PROT_LBPERF ///
	UA_PROT_LBORRES, i(PREGNANCY_ID)  j(lab_visit_num) 
	...
	*rename M07_SUBJID SUBJID
	rename M07_PREGNANCY_ID PREGNANCY_ID
...
keep SUBJID- PREGNANCY_ID DIAGNOSTIC_VSDAT ANC_VISIT_N BP1_SYS_VSORRES BP1_DIA_VSORRES ///
 ...
 *Labeling Variables
	...
	*creating visit number based on date
	sort PREGNANCY_ID visit_date
	bysort PREGNANCY_ID (visit_date): gen bp_visit_num=_n 
	bysort PREGNANCY_ID (visit_date): gen bp_visit_tot=_N //total number of anc visits
	...
	*checking duplicates - this will be caseid and visit date as we expect duplicates of subjectid
	 ...
	 *keep if dup>0
	 *keep SUBJID visit_date dup
	 *export excel using "ReMAPP_Duplicates_14_dec_2021_PID.xlsx"  , sheet("MNH08") firstrow(variables) sheetmodify
	 *restore // restoring old dataset
	********************** 
	...
	*reshaping dataset from long to wide
	...
	BP2_SYS_VSORRES BP2_DIA_VSORRES BP3_SYS_VSORRES BP3_DIA_VSORRES BPAVG_SYS_VSORRES ///
	BPAVG_DIA_VSORRES SPHB_LBORRES, i(PREGNANCY_ID)  j(bp_visit_num)  
	...
	*rename M08_SUBJID SUBJID
	rename M08_PREGNANCY_ID PREGNANCY_ID
...
*M17_CBC_HB_LBORRES* M17_HB_POC_LBORRES*M11_HB_LBORRES M13_POC_HB_VSORRES* M14_HB_POC_LBORRES* M18_CBC_HB_LBORRES* M18_HB_POC_LBORRES*
	foreach var of varlist M07_CBC_HB_LBORRES1 M07_CBC_HB_LBORRES2 M07_CBC_HB_LBORRES3 M07_CBC_HB_LBORRES4 M07_HB_POC_LBORRES1 M07_HB_POC_LBORRES2 M07_HB_POC_LBORRES3 M07_HB_POC_LBORRES4 M01_GEST_AGE_WKS_SCORRES M01_GEST_AGE_MOS_SCORRES M06_US_GA_WEEKS_AGE1 M06_US_GA_WEEKS_AGE2 M06_US_GA_WEEKS_AGE3 M06_US_GA_WEEKS_AGE4 M06_US_GA_WEEKS_AGE5 M06_US_GA_WEEKS_AGE6 M06_US_GA_WEEKS_AGE7 M06_US_GA_WEEKS_AGE8 M06_US_GA_WEEKS_AGE9  M06_US_GA_DAYS_AGE1 M06_US_GA_DAYS_AGE2 M06_US_GA_DAYS_AGE3 M06_US_GA_DAYS_AGE4 M06_US_GA_DAYS_AGE5 M06_US_GA_DAYS_AGE6 M06_US_GA_DAYS_AGE7 M06_US_GA_DAYS_AGE8 M06_US_GA_DAYS_AGE9{
...
*cleaning date variables
	foreach var of varlist M01_LMP_SCDAT M01_EDD_SCDAT M01_ESTIMATED_EDD_SCDAT M01_SCRN_OBSSTDAT {
	replace `var' = "" if(`var' == "-7")
	}
...
************working for missing gestational age
...
*Code for Log File
capture log close
log using "ReMAPP_Data_Cleaning_Part1__pID_Log_14_dec_2021_updated.txt", text  replace

	
*Checking Hemoglobin Results - Listing nonplausible results 

	*MNH07 - ANC Labs
	*CBC Hemoglobin Results
	list PREGNANCY_ID M07_visit_date1 M07_CBC_HB_LBORRES1 if (M07_CBC_HB_LBORRES1<5 | M07_CBC_HB_LBORRES1>=18) & (M07_CBC_HB_LBORRES1 != .)
	list PREGNANCY_ID M07_visit_date2 M07_CBC_HB_LBORRES2 if (M07_CBC_HB_LBORRES2<5 | M07_CBC_HB_LBORRES2>=18) & (M07_CBC_HB_LBORRES2 != .)
	list PREGNANCY_ID M07_visit_date3 M07_CBC_HB_LBORRES3 if (M07_CBC_HB_LBORRES3<5 | M07_CBC_HB_LBORRES3>=18) & (M07_CBC_HB_LBORRES3 != .)
	list PREGNANCY_ID M07_visit_date4 M07_CBC_HB_LBORRES4 if (M07_CBC_HB_LBORRES4<5 | M07_CBC_HB_LBORRES4>=18) & (M07_CBC_HB_LBORRES4 != .)

	
	*Point of Care Hemoglobin Results
	list PREGNANCY_ID M07_visit_date1 M07_HB_POC_LBORRES1 if (M07_HB_POC_LBORRES1<5 | M07_HB_POC_LBORRES1>=18) & (M07_HB_POC_LBORRES1 != .)
	list PREGNANCY_ID M07_visit_date2 M07_HB_POC_LBORRES2 if (M07_HB_POC_LBORRES2<5 | M07_HB_POC_LBORRES2>=18) & (M07_HB_POC_LBORRES2 != .)
	list PREGNANCY_ID M07_visit_date3 M07_HB_POC_LBORRES3 if (M07_HB_POC_LBORRES3<5 | M07_HB_POC_LBORRES3>=18) & (M07_HB_POC_LBORRES3 != .)
	list PREGNANCY_ID M07_visit_date4 M07_HB_POC_LBORRES4 if (M07_HB_POC_LBORRES4<5 | M07_HB_POC_LBORRES4>=18) & (M07_HB_POC_LBORRES4 != .)

	*MNH17 - Unplanned ANC
	*CBC Hemoglobin Results
	list CASEID M17_visit_date1 M17_CBC_HB_LBORRES1 if M17_CBC_HB_LBORRES1<5 | M17_CBC_HB_LBORRES1>=18
	list CASEID M17_visit_date2 M17_CBC_HB_LBORRES2 if M17_CBC_HB_LBORRES2<5 | M17_CBC_HB_LBORRES2>=18
	list CASEID M17_visit_date3 M17_CBC_HB_LBORRES3 if M17_CBC_HB_LBORRES3<5 | M17_CBC_HB_LBORRES3>=18
	
	*Point of Care Hemoglobin Results
	list CASEID M17_visit_date1 M17_HB_POC_LBORRES1 if M17_HB_POC_LBORRES1<5 | M17_HB_POC_LBORRES1>=18
	list CASEID M17_visit_date2 M17_HB_POC_LBORRES2 if M17_HB_POC_LBORRES2<5 | M17_HB_POC_LBORRES2>=18
	list CASEID M17_visit_date3 M17_HB_POC_LBORRES3 if M17_HB_POC_LBORRES3<5 | M17_HB_POC_LBORRES3>=18
	
	*MNH11
	*Hemoglobin at labor and delivery
	list CASEID M11_HB_LBORRES if M11_HB_LBORRES<5 | M11_HB_LBORRES>=18
	
	*MNH13 - PNC
	*Point of Care Hemoglobin Resutls 
	list CASEID M13_visit_date1 M13_POC_HB_VSORRES1 if M13_POC_HB_VSORRES1<5 | M13_POC_HB_VSORRES1>=18
	list CASEID M13_visit_date2 M13_POC_HB_VSORRES2 if M13_POC_HB_VSORRES2<5 | M13_POC_HB_VSORRES2>=18
	list CASEID M13_visit_date3 M13_POC_HB_VSORRES3 if M13_POC_HB_VSORRES3<5 | M13_POC_HB_VSORRES3>=18
	
	*MNH14 - PNC labs
	*Point of Care Hemoglobin Resutls 
	list CASEID M14_visit_date1 M14_HB_POC_LBORRES1 if M14_HB_POC_LBORRES1<5 | M14_HB_POC_LBORRES1>=18
	list CASEID M14_visit_date2 M14_HB_POC_LBORRES2 if M14_HB_POC_LBORRES2<5 | M14_HB_POC_LBORRES2>=18
	list CASEID M14_visit_date3 M14_HB_POC_LBORRES3 if M14_HB_POC_LBORRES3<5 | M14_HB_POC_LBORRES3>=18
	
	*MNH18 - Unplanned PNC
	*CBC Hemoglobin Results
	list CASEID M18_visit_date1 M18_CBC_HB_LBORRES1 if M18_CBC_HB_LBORRES1<5 | M18_CBC_HB_LBORRES1>=18
	list CASEID M18_visit_date2 M18_CBC_HB_LBORRES2 if M18_CBC_HB_LBORRES2<5 | M18_CBC_HB_LBORRES2>=18
	list CASEID M18_visit_date3 M18_CBC_HB_LBORRES3 if M18_CBC_HB_LBORRES3<5 | M18_CBC_HB_LBORRES3>=18
	
	*Point of Care Hemoglobin Results
	list CASEID M18_visit_date1 M18_HB_POC_LBORRES1 if M18_HB_POC_LBORRES1<5 | M18_HB_POC_LBORRES1>=18
	list CASEID M18_visit_date2 M18_HB_POC_LBORRES2 if M18_HB_POC_LBORRES2<5 | M18_HB_POC_LBORRES2>=18
	list CASEID M18_visit_date3 M18_HB_POC_LBORRES3 if M18_HB_POC_LBORRES3<5 | M18_HB_POC_LBORRES3>=18
	
	
*Checking Gestational Age 

	*Participants without any Gestational Estimates 
	list SUBJID PREGNANCY_ID if M01_GEST_AGE_WKS_SCORRES==. & M01_GEST_AGE_MOS_SCORRES==. & M06_US_GA_DAYS_AGE1==. & M06_US_GA_DAYS_AGE2==. & M06_US_GA_DAYS_AGE3==. & M06_US_GA_DAYS_AGE4==. & M06_US_GA_DAYS_AGE5==. & M06_US_GA_DAYS_AGE6==. & M06_US_GA_DAYS_AGE7==. & M06_US_GA_DAYS_AGE8==. & M06_US_GA_DAYS_AGE9==.  
	
	*Gestational Age out of range
	list SUBJID PREGNANCY_ID M01_GEST_AGE_WKS_SCORRES if (M01_GEST_AGE_WKS_SCORRES<4 | M01_GEST_AGE_WKS_SCORRES>=38) & M01_GEST_AGE_WKS_SCORRES!= .  //should this be less when should they be recruited?
	list SUBJID PREGNANCY_ID M01_GEST_AGE_MOS_SCORRES if (M01_GEST_AGE_MOS_SCORRES<1 | M01_GEST_AGE_MOS_SCORRES>=10) & M01_GEST_AGE_MOS_SCORRES!= .
	
	*Missing EDD or date of last period 
	list PREGNANCY_ID SUBJID if M01_LMP_SCDAT==. | M01_EDD_SCDAT==.
    
	****Both EDD and date of LMP is missing
	list PREGNANCY_ID SUBJID if M01_LMP_SCDAT==. & M01_EDD_SCDAT==.

	*Last Period Date not feasible 
	list SUBJID M01_LMP_SCDAT if M01_LMP_SCDAT > td(23nov2021) & M01_LMP_SCDAT!=.
	
	*EDD Date not feasible 
	list SUBJID M01_EDD_SCDAT if M01_EDD_SCDAT > td(23nov2021)+270 & M01_EDD_SCDAT!= .
	
	

log close

*translate "ReMAPP_Data_Cleaning_Log_part1_14_dec_2021_Preg_ID.txt" "ReMAPP_Data_Cleaning_Log_29_nov_2021_updated.pdf"
