...
*************************************************
		 ...
		 ***Since we have multiple measures of hemoglobin we need to separate datasets - can append datasets after
			...
			*keeping relevant variables for CBC
			keep SUBJID PREGNANCY_ID visit_date hemoglobin hem_type lmp_cal
		 
			*Saving as temporary file for append 
			tempfile cbc
			sort PREGNANCY_ID
			save cbc_PID,replace
		...
		****Appending Datasets 
		use cbc_PID, clear 
			...
			append using poc_PID 
			append using sphb_PID 
		...
		******************************************************************
			capture log close 
			*log using "ReMAPP_ANC_Hemoglobin_part_2_14_dec_2021_preg_ID.txt", text  replace
			log using "ReMAPP_ANC_Hemoglobin_part_2_23_dec_2021_preg_ID.txt", text  replace

			**************checking error in gest age
            list SUBJID PREGNANCY_ID  gestage if gestage< 0
		...
		********************************************
			...
			*Scatter plot
		 twoway (scatter hemoglobin gestage if hem_type=="CBC", color(lavender)) ///
			 (scatter hemoglobin gestage if hem_type=="POC", color(navy)) ///
			 (scatter hemoglobin gestage if hem_type=="SpHb", color(eltgreen)), ///
			 ...
			 *graph export scatter_hb_gestage_preg_ID.png, replace 
			 graph export scatter_hb_gestage_preg_ID_new.png, replace 

			
			**All Observatons - CBC, POC, SpHb methods
			*histogram
			histogram hemoglobin 
				*graph export all_hemoglobin_preg_ID.png,replace  
				graph export all_hemoglobin_preg_ID_new.png,replace  

			*histogram by trimester - same graph
			twoway (histogram hemoglobin if tri_lab==1, width(0.5) color(lavender)) ///
			   (histogram hemoglobin  if tri_lab==2, width(0.5) color(navy)) ///
			   (histogram hemoglobin if tri_lab==3, width(0.5) color(eltgreen)), ///
			  legend(order(1 "Trimester 1" 2 "Trimester 2" 3 "Trimester 3" )) 
				*graph export all_hemoglobin_tri1_preg_ID.png, replace 
     			graph export all_hemoglobin_tri1_preg_ID_new.png, replace 

			 *histrogram by trimester - different graphs
			 twoway histogram hemoglobin, by(tri_lab) 
				*graph export all_hemoglobin_tri2_preg_ID.png, replace
                graph export all_hemoglobin_tri2_preg_ID_new.png, replace
			...
			*histogram - CBC Only
			histogram hemoglobin 
				*graph export cbc_hemoglobin_preg_ID.png, replace
				graph export cbc_hemoglobin_preg_ID_new.png, replace
			...
			*histogram by trimester - same graph
			twoway (histogram hemoglobin if tri_lab==1, width(0.5) color(lavender)) ///
			   ...
			   (histogram hemoglobin if tri_lab==3, width(0.5) color(eltgreen)), ///
			  legend(order(1 "Trimester 1" 2 "Trimester 2" 3 "Trimester 3" )) 
				*graph export cbc_hemoglobin_tri1_preg_ID.png, replace 
                 graph export cbc_hemoglobin_tri1_preg_ID_new.png, replace 
			  			  
			 *histrogram by trimester - different graphs
			 twoway histogram hemoglobin, by(tri_lab) 
				*graph export cbc_hemoglobin_tri2_preg_ID.png, replace 
			     graph export cbc_hemoglobin_tri2_preg_ID_new.png, replace
			     restore 
			...
			*histogram - SpHb Only
			histogram hemoglobin 
				*graph export SpHb_hemoglobin_preg_ID.png, replace
				graph export SpHb_hemoglobin_preg_ID_new.png, replace
			...
			*histogram by trimester - same graph
			twoway (histogram hemoglobin if tri_lab==1, width(0.5) color(lavender)) ///
			   ...
			   (histogram hemoglobin if tri_lab==3, width(0.5) color(eltgreen)), ///
			  legend(order(1 "Trimester 1" 2 "Trimester 2" 3 "Trimester 3" )) 
				*graph export SpHb_hemoglobin_tri1_preg_ID.png, replace 
				graph export SpHb_hemoglobin_tri1_preg_ID_new.png, replace 
	  
			 *histrogram by trimester - different graphs
			 twoway histogram hemoglobin, by(tri_lab) 
				*graph export SpHb_hemoglobin_tri2_preg_ID.png, replace 
                 graph export SpHb_hemoglobin_tri2_preg_ID_new.png, replace 
			...
			*histogram - POC Only
			histogram hemoglobin 
				graph export poc_hemoglobin_preg_ID.png, replace
			...
			*histogram by trimester - same graph
			twoway (histogram hemoglobin if tri_lab==1, width(1) color(green)) ///
			  ...
			  legend(order(1 "Trimester 1" 2 "Trimester 2" 3 "Trimester 3" )) 
					graph export poc_hemoglobin_tri1_preg_ID.png, replace 
			  
			 *histrogram by trimester - different graphs
			 twoway histogram hemoglobin, by(tri_lab) 
					graph export poc_hemoglobin_tri2_preg_ID.png, replace  
